...
async def button_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("gen_ud_"):
        ...
        def _gen():
            ...
            asyncio.run_coroutine_threadsafe(
                bot.send_message(chat_id, f"Generate UD {ud_num} selesai! Stok: {count_stok(ud_num)}"), main_loop)
            active_gen_task.pop(uid, None)
        t = threading.Thread(target=_gen, daemon=True); t.start()
        active_gen_task[uid] = {"stop": stop_evt, "thread": t}
        await q.edit_message_text(f"Generate UD {ud_num} dimulai! Target: {needed} video", reply_markup=main_menu_kb(uid)); return
